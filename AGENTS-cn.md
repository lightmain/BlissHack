...
每个开发阶段必须依次执行以下流程，不得跳过或调换顺序：

1. 先由 subagent 编写或补充该阶段的测试；新增行为优先建立预期失败测试，
   重构阶段建立 characterization tests。
2. 主 Agent 在测试基线确定后进行开发实现。
3. 实现完成后由 subagent 独立执行代码审查。
4. 主 Agent 根据审查 findings 修正实现，再运行该阶段的完整验证。

测试编写和代码审查不得由主 Agent 的自检替代。如果当前环境无法创建
subagent，应暂停该阶段并向用户说明，不能静默跳过流程。
每完成一个阶段的稳定状态下，以及其他适合进行git commit的时机，进行git commit存档。
